...
return {
    ...
    config = function()
        ...
        vim.keymap.set('n', keys.prev_file, function() next_harpoon(harpoon:list(), true) end,
            {desc = desc('Previous file')})
        vim.keymap.set('n', keys.next_file, function() next_harpoon(harpoon:list(), false) end,
            {desc = desc('Next file')})
